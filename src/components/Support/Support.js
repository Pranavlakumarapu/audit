...
export default function Support() {
  ...
  const openVideoPopup = (event) => {
    const src = event.currentTarget.getAttribute('data-src');
    videoRef.current.src = src;
    document.querySelector('.popup-video').style.display = 'block';
  };

  const closeVideoPopup = (event) => {
    event.preventDefault();
    document.querySelector('.popup-video').style.display = 'none';
    videoRef.current.src = ''; // reset the video src to stop playback
  };
    ...
    <div>
      <div className='supportbody'>
        <div className='supportcontainer'>
          ...
          {support.map((supports, index) => (
  <div key={index} className='video'>
    <video
      src={supports.video}
      onClick={openVideoPopup}
      data-src={supports.video}
    ></video>
    <div className='videodescription'>    <p>{supports.desc}</p></div> 
  </div>
))}
          </div>

          <div className='popup-video'>
            <span onClick={closeVideoPopup}>&times;</span>
            <video
              ref={videoRef}
              src=''
              autoPlay
